...
export function translate(
  ...
  const wholeFileNode = Parser.parse(deobfuscatedSource, {
    sourceType: "script",
  });
  ...
  const randomizerPatchFunctionExpression = Parser.parse(clairvoyancePatch, {
    sourceType: "script",
  }).body[0].expression;
  ...
  // replace the identifiers and values in the patched function
  const sectorIndexPatchFunctionExpression = Parser.parse(snapPatch, {
    sourceType: "script",
  }).body[0].expression;
  ...
  // replace the identifiers and values in the patched function
  const afterScriptLoadFunctionExpression = Parser.parse(reinitPatch, {
    sourceType: "script",
  }).body[0].expression;
  ...
  const patchedAsSource = generate(wholeFileNode, { format: FORMAT_MINIFY });
  ...
  const patchedAsUserscript = scripterTemplate.replace(
